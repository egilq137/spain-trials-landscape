...
- **Sponsor structure:** industry vs. academic/public share; top sponsors.
  *Share answered; top sponsors still to do.* The database stores the sponsor
  ...
  cannot, and the rest is **`Unclassified` rather than forced** — 576 trials,
  4.9%. **Industry 9,475 (80.1%), academic or public 1,783 (15.1%).**
  ...
  **Industry's share is rising, 77.1% → 85.8%** — but the unclassified share
  fell from 7.8% to 2.7% over the same window as registry names got more
  complete, so part of that is trials becoming classifiable. Measured against
  classified trials only, the rise is **83.6% → 88.1%**, about half the crude
  ...
  *Top sponsors, once the families are merged.* 2,957 sponsors is a list of
  legal entities, not of companies: **Novartis files under 10 spellings and
  Roche under 14**, so a ranking of the raw column answers "which legal entity
  signed the most protocols" rather than "which company is behind the most
  Spanish trials". Merged: **Novartis 537, Roche 481, AstraZeneca 385, MSD 351,
  Janssen 318.** Unmerged the list is led by `AstraZeneca AB` with 348 — an
  entity, not a company — and Roche does not appear in the top five at all.

  Candidates were generated by blocking on the leading token of the folded
  name, and **every group was read before it was accepted**, which is what
  caught all four of these:

  - `Merck Sharp & Dohme` (351, US) and `Merck KGaA` (66, German) are
    different companies that split in 1917. A `Merck` rule merges them, and
    also swallows `Merckle GmbH`, a third company again.
  - `Roche Farma S.A.` and `F. Hoffmann-La Roche` block *apart* — one name
    starts with `F.` — and are one company. Several Spanish entries say so
    outright: "que actúa como representante de F. Hoffmann-La Roche".
  - `Lilly S.A.` (101) and `Eli Lilly & Co.` (95) block apart for the same
    reason. This is the pair the Phase 2 handoff flagged.
  - The block keys `laboratorios` and `fundacion` gather 29 unrelated Spanish
    laboratories and 137 unrelated foundations. **A common word is not a
    family**, and neither block produced a single merge.

  REEC often states the relationship itself — `Medivation Inc. (a wholly owned
  subsidiary of Pfizer)`, `Millennium Pharmaceuticals, Inc., a wholly owned
  subsidiary of Takeda` — so a pattern searching anywhere in the name picks up
  acquisitions for free. `Celgene` is deliberately **not** merged into Bristol
  Myers Squibb: half this window predates the 2019 acquisition, and merging
  would credit BMS with trials it did not run at a time it did not own them.

  The two tables reinforce each other: family membership is itself evidence of
  being a company, so `Novartis Farmacéutica` and `Janssen R&D Ireland` — no
  legal form, no institutional word — become classifiable, worth 49 trials.
  It is consulted *after* the markers, not before: a marker is direct evidence
  about the entity, family membership is an inference from its name, and a
  hypothetical `Fundación Servier` is a foundation whatever the pattern says.

  **`docs/sponsor-families.html` is where the merges get read**, generated by
  `run_analysis.py` so it cannot drift from the rules it documents. It has two
  tables, and the second is the one that earns it: **every sponsor that joined
  no family, biggest first**. A rule cannot report the merge it failed to
  make, so the only way to find a missing family is to read what was left
  alone. Scanning it produced a second pass of nine: `Genentech` → Roche,
  `Genzyme` → Sanofi, `MedImmune` → AstraZeneca and `Millennium` → Takeda,
  which share no token with their parent and could only be found by knowing
  about them; and `UCB` (7 spellings), `MedSIR` (4), `BeiGene`/`BeOne
  Medicines` (4, a company that renamed itself), `Eisai` (2) and `ViiV
  Healthcare` (2), which are one organisation spelling its own name several
  ways. That is 28 families over 4,870 trials, and it moved Roche from 429 to
  481 and Sanofi from 205 to 231.

  **When an acquisition merges and when it does not.** Two cases merge: an arm
  the parent owned for the whole window — Genentech has been Roche's since
  2009, Genzyme Sanofi's since 2011, MedImmune AstraZeneca's since 2007 — and
  a name that declares its own parent, where the registry states the
  relationship rather than us inferring it. Companies bought *during* the
  window and still filing under their own name are left alone: Celgene,
  Seagen, Actelion, Kite and Baxalta each ran trials here for years before
  their buyer owned them. The rule is the window, not the present day.
...
- [x] Sponsor structure, part 2: top sponsors, with the corporate families
      merged (`FAMILIES` in `analysis/sponsors.py`), and `docs/sponsor-families.html`
      as the page the merges are read on
