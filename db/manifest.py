...
db/rules.py says what we *would* change. This says what we *did*, on this run,
against this cache. The two are not the same thing and the difference is the
point: the rules carry counts measured in September 2026, and a REEC refresh
can move every one of them.

Why it exists at all, given that tests/test_rules.py already re-measures the
counts:

  * **Provenance.** db/rules.py claims "the rules plus the load manifest are
    ...
    getting back to it means re-running the loader and diffing. The manifest is
...
class Manifest:
    ...
    def report(self):
        """The manifest as text, grouped by field, most-changed field first."""
        if not self._counts:
            return "Load manifest — {:,} records, nothing changed".format(
                self.records)
        ...
        lines = ["Load manifest — {:,} records".format(self.records), ""]
