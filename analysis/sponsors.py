...
`Unclassified` and is shown as such: 644 trials, 5.4%. Forcing them into one
...
trying not to commit. Industry 9,407 (79.5%), academic or public 1,783
(15.1%).

**And the unclassified share is not constant**, which matters more than its
size: it falls from 8.8% of trials in 2013 to 2.7% in 2026 as registry names
...
def classify(name):
    """One of CLASSES. The hand list wins, then individuals, then markers."""
    ...
    if industry == academic:
        # Both or neither. A tie-break here would be a rule invented to avoid
        # saying "I do not know", and the sponsors it would decide are the
        # ones nobody checked.
        return UNCLASSIFIED
...
def share_figure(shares):
    ...
    only. The unclassified share falls from 8.8% to 2.7% across the window --
    ...
    return _layout(
        ...
        "<br>The muted line removes the unclassified, which shrank from 8.8% "
